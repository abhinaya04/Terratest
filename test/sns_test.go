...
func TestTerraformAwsSNSExample(t *testing.T) {
	t.Parallel()

	// Give this S3 Bucket a unique ID for a name tag so we can distinguish it from any other Buckets provisioned
	// in your AWS account
	expectedName := fmt.Sprintf("sns-event-topic-%s", strings.ToLower(random.UniqueId()))
	...
	approvedRegions := []string{"ap-south-1"}
	...
	terraformOptions := terraform.WithDefaultRetryableErrors(t, &terraform.Options{
		...
		Vars: map[string]interface{}{
			"tag_topic_name":        expectedName,
			// "tag_topic_environment": expectedEnvironment,
		},
	...
	// topic_arn := terraform.Output(t, terraformOptions, "sns_topic_arn")
	...
	// aws.AssertSNSTopicPolicyExists(t, awsRegion, sns_topic_arn)
